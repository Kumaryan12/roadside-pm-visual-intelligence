...
def build_sequence_arrays(seq_df, row_features, feature_cols, embeddings, T=7):
    ...
    for i, r in seq_df.iterrows():
        ...
        emb_seq = embeddings[ids]

        X_eng.append(eng_seq)
        X_emb.append(emb_seq)
    ...
    X = np.concatenate([X_emb, X_eng], axis=2)
    seq_kept = seq_df.loc[keep_rows].reset_index(drop=True)

    return X, y, seq_kept, X_emb.shape[2], X_eng.shape[2]
...
def train_model(model, train_loader, val_loader, device, epochs, lr, patience):
        ...
        for xb, yb in train_loader:
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(
                    ...
                    "Check NaNs/Infs in X/y or reduce learning rate."
                )
    ...
    if best_state is None:
        raise RuntimeError(
            "Training failed: validation loss was never finite. "
            "Check feature NaNs/Infs, target scaling, or reduce learning rate."
        )
...
def main():
    ...
    X, y, seq_kept, embedding_dim, engineered_dim = build_sequence_arrays(
        seq_df=seq_df,
    ...
    print("X finite:", np.isfinite(X_scaled).mean())
    print("y finite:", np.isfinite(y_scaled).mean())
    print("X nan count:", np.isnan(X_scaled).sum())
    print("y nan count:", np.isnan(y_scaled).sum())
    print("X min/max:", np.nanmin(X_scaled), np.nanmax(X_scaled))
    print("y min/max:", np.nanmin(y_scaled), np.nanmax(y_scaled))

    train_ds = T7FusionDataset(X_scaled[train_idx], y_scaled[train_idx])
    val_ds = T7FusionDataset(X_scaled[val_idx], y_scaled[val_idx])
    test_ds = T7FusionDataset(X_scaled[test_idx], y_scaled[test_idx])
    ...
    arch_name = f"{args.rnn_type}{'_bidir' if args.bidirectional else ''}"

    print("=" * 90)
    print("TRAQID T7 MULTIMODAL RNN NO-PM INPUTS")
    print("=" * 90)
    ...
    print("Total input dim:", input_dim)
    ...
    metrics["model"] = f"t7_multimodal_{arch_name}_no_pm_inputs"
    ...
    print("\n" + "=" * 90)
    print("TRAQID T7 MULTIMODAL RNN COMPLETE")
    print("=" * 90)
