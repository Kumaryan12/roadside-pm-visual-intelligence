...
RESULTS_OUT = OUTDIR / "image_dominant_purged_block_results.csv"
PRED_OUT = OUTDIR / "image_dominant_purged_block_predictions.csv"
...
def make_purged_block_split(df, date_col="date", test_frac=0.20, purge_days=2):
    """
    Chronological block split:
    - Sort unique dates.
    - Last test_frac dates become test block.
    - purge_days dates immediately before test are removed.
    - Everything before purge is train.
    """

    unique_dates = pd.to_datetime(pd.Series(df[date_col].unique())).sort_values().reset_index(drop=True)

    n_dates = len(unique_dates)
    n_test_dates = max(1, int(round(n_dates * test_frac)))

    test_dates = unique_dates.iloc[-n_test_dates:]
    ...
    train_mask = df_dates < purge_start
    purge_mask = (df_dates >= purge_start) & (df_dates < test_start)
    test_mask = df_dates >= test_start
    ...
    print("Train dates:", train["date"].nunique(), train["date"].min(), "to", train["date"].max())
    print("Purge dates:", purge["date"].nunique(), purge["date"].min() if len(purge) else None, "to", purge["date"].max() if len(purge) else None)
    print("Test dates:", test["date"].nunique(), test["date"].min(), "to", test["date"].max())
    print("Rows:", {"train": len(train), "purge": len(purge), "test": len(test)})

    return train, purge, test
...
def main():
    ...
    df = add_weak_environment_features(df)
    ...
    train, purge, test = make_purged_block_split(
        df,
        date_col="date",
        test_frac=0.20,
        purge_days=2,
    ...
    if len(train) == 0 or len(test) == 0:
        raise ValueError("Train or test split is empty. Adjust test_frac/purge_days.")

    y_train = train[TARGET].values.astype(float)
    y_test = test[TARGET].values.astype(float)

    # ========================================================
    # Embeddings aligned by row_id
    # ========================================================

    all_emb = load_resnet_embeddings(df)

    # Easier: create row position lookup from df order
    ...
    feature_sets = {
        ...
        "image_temp_humidity": {
            ...
            "use_resnet": True,
            "pca": 32,
            "description": "Image features + Temperature/Humidity",
        },
        "image_temp_humidity_daynight": {
            ...
            "use_resnet": True,
            "pca": 32,
            "description": "Image features + Temperature/Humidity + Day/Night",
        },
        "weak_env_only": {
            "numeric": weak_env,
            "categorical": daynight_cat,
            "use_resnet": False,
            "pca": None,
            "description": "Weak environment only: Temperature/Humidity + Day/Night",
        },
    }
    ...
    for fs_name, cfg in feature_sets.items():
        ...
        for model_name, model in models.items():
            ...
            row = {
                ...
                "resnet_pca": cfg["pca"],
                "uses_temp_humidity": any(f in cfg["numeric"] for f in weak_env),
    ...
    print("\n" + "=" * 100)
    print("FINAL PURGED BLOCK RESULTS:")
