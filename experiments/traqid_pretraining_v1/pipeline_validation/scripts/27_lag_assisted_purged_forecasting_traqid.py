...
RESULTS_OUT = OUTDIR / "lag_assisted_purged_traqid_results.csv"
PRED_OUT = OUTDIR / "lag_assisted_purged_traqid_predictions.csv"
SUMMARY_OUT = OUTDIR / "lag_assisted_purged_traqid_summary.csv"
...
# ============================================================
# Lag feature engineering — IMPORTANT: timestamp-level, no row leakage
# ============================================================

def add_timestamp_level_lag_features(df):
    """
    Build lags at unique timestamp level, then merge back.

    This avoids same-timestamp leakage where multiple images at the same timestamp
    could otherwise use each other's PM2.5 as a lag.
    """
    ...
    time_df = (
        df
        .groupby("created_at_parsed")
        .agg(
            PM25_time=("PM2.5", "mean"),
            PM10_time=("PM10", "mean"),
            Temperature_time=("Temperature", "mean"),
            Humidity_time=("Humidity", "mean"),
        )
        .reset_index()
    ...
    # Lags by timestamp step, not by row.
    for lag in [1, 2, 3, 6, 12]:
        time_df[f"pm25_lag{lag}"] = time_df["PM25_time"].shift(lag)
        time_df[f"pm10_lag{lag}"] = time_df["PM10_time"].shift(lag)
        time_df[f"temp_lag{lag}"] = time_df["Temperature_time"].shift(lag)
        time_df[f"humidity_lag{lag}"] = time_df["Humidity_time"].shift(lag)

    # Rolling features strictly from previous timestamps
    shifted_pm25 = time_df["PM25_time"].shift(1)
    shifted_pm10 = time_df["PM10_time"].shift(1)
    shifted_temp = time_df["Temperature_time"].shift(1)
    shifted_humidity = time_df["Humidity_time"].shift(1)

    for win in [3, 6, 12]:
        time_df[f"pm25_roll_mean_{win}"] = shifted_pm25.rolling(win, min_periods=1).mean()
        time_df[f"pm25_roll_std_{win}"] = shifted_pm25.rolling(win, min_periods=2).std()
        time_df[f"pm10_roll_mean_{win}"] = shifted_pm10.rolling(win, min_periods=1).mean()
        time_df[f"pm10_roll_std_{win}"] = shifted_pm10.rolling(win, min_periods=2).std()
        time_df[f"temp_roll_mean_{win}"] = shifted_temp.rolling(win, min_periods=1).mean()
        time_df[f"humidity_roll_mean_{win}"] = shifted_humidity.rolling(win, min_periods=1).mean()

    # Slope proxies from previous values only
    time_df["pm25_slope_1"] = time_df["pm25_lag1"] - time_df["pm25_lag2"]
    time_df["pm25_slope_3"] = time_df["pm25_lag1"] - time_df["pm25_lag3"]
    time_df["pm10_slope_1"] = time_df["pm10_lag1"] - time_df["pm10_lag2"]
    time_df["pm10_slope_3"] = time_df["pm10_lag1"] - time_df["pm10_lag3"]

    lag_cols = [c for c in time_df.columns if c not in [
        "created_at_parsed",
        "PM25_time",
        "PM10_time",
        "Temperature_time",
    ...
    df = df.merge(
        time_df[["created_at_parsed"] + lag_cols],
        on="created_at_parsed",
        how="left",
    )

    print("\nTimestamp-level lag features added.")
    print("Unique timestamps:", len(time_df))
...
def build_dataset():
    ...
    df = df.dropna(subset=[TARGET, "PM10", "Temperature", "Humidity", "created_at", "aqi_cat"]).copy()
    ...
    df, lag_cols = add_timestamp_level_lag_features(df)
...
def get_feature_groups(lag_cols):
    yolo_count_features = [
        "car_count",
        "bus_count",
        "truck_count",
    ...
    image_all = yolo_count_features + yolo_advanced_features + road_features + yolo_road_interactions
    ...
    # Separate lag groups
    pm_lag_cols = [c for c in lag_cols if c.startswith("pm25_") or c.startswith("pm10_")]
    weather_lag_cols = [c for c in lag_cols if c.startswith("temp_") or c.startswith("humidity_")]

    return image_all, weather_current, daynight_cat, pm_lag_cols, weather_lag_cols


# ============================================================
# ResNet + matrix helpers
# ============================================================
...
def main():
    df, lag_cols = build_dataset()

    image_all, weather_current, daynight_cat, pm_lag_cols, weather_lag_cols = get_feature_groups(lag_cols)

    all_numeric = sorted(set(image_all + weather_current + pm_lag_cols + weather_lag_cols))
    df = clean_numeric(df, all_numeric)

    # Drop rows where key PM lag does not exist. This avoids early timestamp artifacts.
    required_lags = ["pm25_lag1", "pm10_lag1"]
    df = df.dropna(subset=[c for c in required_lags if c in df.columns]).copy()

    print("\nAfter dropping rows without lag1:", len(df))
    print("Unique dates:", df["date"].nunique())
    ...
    feature_sets = {
        ...
        "pm_lag_only": {
            ...
            "use_resnet": False,
            "pca": None,
            "description": "Past PM2.5/PM10 lags and rolling history only",
        },

        "pm_lag_weather_history": {
            "numeric": pm_lag_cols + weather_lag_cols + weather_current,
            "categorical": daynight_cat,
            "use_resnet": False,
            "pca": None,
            "description": "PM history + weather history/current + Day/Night",
        },
        ...
        "image_only": {
            ...
            "use_resnet": True,
            "pca": 64,
            "description": "Image-only: YOLO + road + ResNet50",
        },

        "image_plus_pm_lag": {
            "numeric": image_all + pm_lag_cols,
            "categorical": [],
            "use_resnet": True,
            "pca": 32,
            "description": "Image features + PM history",
        },

        "image_plus_pm_lag_weather": {
            "numeric": image_all + pm_lag_cols + weather_lag_cols + weather_current,
            "categorical": daynight_cat,
            "use_resnet": True,
            "pca": 32,
            "description": "Image features + PM history + weather history/current + Day/Night",
        },
    }
        ...
        for fs_name, cfg in feature_sets.items():
            if fs_name == "mean_baseline":
                ...
                row = {
                    ...
                    "train_pm25_mean": float(np.mean(y_train)),
                    "elapsed_sec": 0.0,
                }
    ...
    summary = (
        ...
        .agg(
            ...
            mean_train_pm25=("train_pm25_mean", "mean"),
    ...
    print("\n" + "=" * 100)
    print("LAG-ASSISTED PURGED TRAQID SUMMARY:")
