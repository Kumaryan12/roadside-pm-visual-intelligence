...
OUT_RESULTS = REPORTS / "pm25_temporal_t7_fair_random2fold_results.csv"
OUT_PREDS = REPORTS / "pm25_temporal_t7_fair_random2fold_predictions.csv"
OUT_FEATURES = REPORTS / "pm25_temporal_t7_fair_random2fold_feature_groups.json"
...
def make_temporal_table(df, feature_cols, target_col, t=7):
    ...
    return pd.DataFrame(rows), np.array(y, dtype=float), np.array(source_indices, dtype=int)
...
def residual_fusion_oof(X_base, X_res, y, base_model_name, residual_model_name, log_target=False):
    cv = KFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    ...
    for fold, (train_idx, test_idx) in enumerate(cv.split(X_base), start=1):
        ...
        residual_pipe = Pipeline([
            ("preprocess", make_preprocessor(Xr_train)),
            ("model", model_factory(residual_model_name)),
        ])

        residual_pipe.fit(Xr_train, train_residual)
        residual_pred = residual_pipe.predict(Xr_test)

        pred = np.clip(base_test_real + residual_pred, 0, None)

        final_pred[test_idx] = pred
        base_oof[test_idx] = np.clip(base_test_real, 0, None)
        residual_oof[test_idx] = residual_pred
...
def main():
    ...
    for fs_name, (X, y, source_indices) in temporal_tables.items():
        for model_name in model_names:
            for log_target in [False, True]:
                exp = f"direct_T{T}_random2fold_{fs_name}_{model_name}_{'log1p' if log_target else 'raw'}"
    ...
    for res_name, X_res in residual_sets.items():
        for base_model in base_models:
            for residual_model in residual_models:
                for log_target in [False, True]:
                    exp = f"residual_T{T}_random2fold_{res_name}_base-{base_model}_res-{residual_model}_{'log1p' if log_target else 'raw'}"
    ...
    print("\nFinal T7 random 2-fold fair results:")
