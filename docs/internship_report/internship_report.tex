...
\newcommand{\homeinstitution}{National Institute of Technology Goa}
\newcommand{\hostinstitution}{Indian Institute of Technology Madras}
\newcommand{\supervisor}{Prof.\ Raghunathan Rengaswamy}
\newcommand{\internshipperiod}{May--July 2026}
\newcommand{\submissiondate}{July 2026}
\newcommand{\projecttitle}{Background-Anchored Multimodal Modelling of Roadside PM$_{2.5}$}
\newcommand{\projectrepository}{https://github.com/Kumaryan12/roadside-pm-visual-intelligence}
...
\begin{titlepage}
  ...
  \begin{tabular}{rl}
    ...
    \textbf{Home institute:} & \homeinstitution\\
    \textbf{Host institute:} & \hostinstitution\\
  ...
  {\small\textbf{Project repository: }
  \href{\projectrepository}{\texttt{Kumaryan12/roadside-pm-visual-intelligence}}\par}
  \vspace{8mm}
\end{titlepage}
...
I express my sincere gratitude to \supervisor\ for his guidance, technical
questions and insistence on a physically defensible interpretation of the
modelling problem. I am especially grateful to Dr.\ Sreejith Chakrapani,
Postdoctoral Researcher at the Indian Institute of Technology Madras, and
Navaneethakrishnan V, Project Associate and M.S.\ by Research scholar at the
Indian Institute of Technology Madras, for their generous technical assistance,
practical guidance and continued support throughout this project. I thank the
data collection team and the contributors to the TRAQID and MUMMA datasets for
making this work possible. I also acknowledge the open-source communities
behind PyTorch, scikit-learn, OpenStreetMap, Ultralytics, Hugging Face,
Copernicus CAMS, NASA MERRA-2 and the geospatial data services used in this
project. I am grateful to \hostinstitution\ for hosting the internship and to
\homeinstitution\ for supporting the academic programme.
...
\section{Original source-aware research direction}
The internship did not begin with the final architecture already fixed. The
original question was whether roadside particle behaviour---including a
provisional effective-density proxy---could be explained using traffic, road
condition, meteorology, gases and geospatial context:
\begin{equation}
  \rho_{\mathrm{eff}} =
  f(\mathrm{traffic},\mathrm{road},\mathrm{weather},\mathrm{gases},\mathrm{OSM}).
\end{equation}
Candidate mechanisms included exhaust, brake and tyre wear, road-dust
resuspension, secondary aerosol and mixtures of these processes. The available
measurements did not contain chemically speciated source contributions.
Consequently, the project adopted the terms \emph{source-aware feature},
\emph{source proxy}, \emph{particle-regime indicator} and \emph{local pollution
indicator}; it does not present exact chemical source apportionment.

The technical and scientific sequence was:
\begin{align*}
\text{source-aware modelling}
&\rightarrow \text{visual feature extraction}
\rightarrow \text{\pmtwo\ prediction}\\
&\rightarrow \text{validation audit}
\rightarrow \text{atmospheric background decomposition}.
\end{align*}
This evolution is central to the report. Later validation evidence and
supervisor feedback changed the interpretation of earlier models instead of
being retrofitted to a predetermined narrative.
...
\begin{itemize}
  ...
  \item complete YOLO traffic, road segmentation, OSM, AlphaEarth and atmospheric
        feature pipelines;
  \item an IDD-fine-tuned YOLO11m traffic detector and an Indian-road-tuned
        SegFormer model with IoU 0.893 and Dice 0.939;
  \item controlled road-area and white-balance experiments, including a
        \SI{4}{\square\metre} patch recovered as approximately
        \SI{3.72}{\square\metre};
...
This report covers the initial 281-row synchronization and modelling pilot, the
historical TRAQID reproduction, the two-date MUMMA development set, the
complete five-day MUMMA experiments, the newer CAMS and MERRA-2
...
\section{Initial synchronized sensor--image pilot}
The first substantial engineering task was constructing a synchronized dataset,
not training a neural network. The early MUMMA pilot contained 281 sensor rows,
252 rows with usable corresponding images and 756 successfully preprocessed
frames from lenses 1, 4 and 6. Sensor timestamps and video run metadata were
used to locate the corresponding video offset. Frames were then extracted,
rotated into a common orientation, cropped, masked to remove the platform and
assigned persistent sample and run identifiers. Raw videos were never modified.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{synchronization_pipeline.png}
  \caption{Evolution of the synchronized data pipeline from the compact pilot
  to the canonical five-day dataset.}
  \label{fig:synchronization}
\end{figure}

This pilot established the invariant used throughout the later project:
sensor, image, traffic, road and geospatial data must join through explicit
identity and timestamp fields rather than through implicit row ordering.
Later orientation audits replaced lens 4 with lens 2 in the canonical
three-view configuration, yielding lenses 1, 2 and 6.
...
\section{MUMMA two-date development dataset}
The subsequent two-date development set used 2,167 aligned records from 1 and
3 February 2026. It
established:
\begin{itemize}
...
YOLO11m was fine-tuned on the IDD 15-class Indian-road dataset and then applied
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{visual_pipeline_examples.png}
  \caption{Representative visual processing outputs: calibrated frame, IDD
  traffic detections, SegFormer road overlay and depth-assisted geometry.}
  \label{fig:visualexamples}
\end{figure}

\section{Road appearance features}
Road extraction initially used OpenCV colour/texture thresholds and CLIP-based
semantic experiments. These approaches were unstable under shadows, glare,
road-colour variation, occlusion and complex Indian scenes. The project
therefore moved to a Cityscapes-pretrained SegFormer model, which was fine-tuned
for binary Indian-road segmentation. Its independently evaluated road-mask
performance was approximately IoU 0.893 and Dice 0.939. This was one of the
strongest component-level computer-vision results of the internship.

The SegFormer-B0 road model was run on the calibrated road-facing lenses 1 and 6.
...
metric area. For road pixel $(u,v)$ with estimated metric depth $z$, camera
intrinsics mapped the pixel into 3D:
\begin{equation}
  x=\frac{(u-c_x)z}{f_x}, \qquad
  y=\frac{(v-c_y)z}{f_y}.
\end{equation}
Neighbouring road points were connected as triangles and their areas summed.
Occluded road beneath vehicles was then explored through three successive
heuristics. Version 1 used a vehicle-footprint prior, box visibility,
road-contact confidence and conservative scaling. Removing road-contact
confidence in version 2 overestimated distant vehicles. Version 3 therefore
introduced depth gating:
\begin{equation}
  A_{\mathrm{hidden}} =
  A_{\mathrm{footprint}} V_{\mathrm{bbox}} D_{\mathrm{factor}}
  C_{\mathrm{conservative}} .
\end{equation}

A controlled \SI{2}{\metre}$\times$\SI{2}{\metre} marked patch had true area
\SI{4.00}{\square\metre}; the pipeline estimated approximately
\SI{3.72}{\square\metre}, an error of about 6.94\%. Despite that promising
controlled check, the complete five-day extraction remained provisional. It
produced a value for all \num{6873} samples, with mean
...
\section{White-balance calibration}
Road colour and dryness proxies are sensitive to illumination and camera colour
processing. Automatic, daylight and fixed-6000\,K white balance were compared
through the coefficient of variation of the road-brown feature. The values were
27.53\%, 25.44\% and 12.17\%, respectively. Fixed 6000\,K was selected because
it was the most repeatable tested setting, not because it establishes an
objectively correct physical road colour.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{road_engineering_validation.png}
  \caption{Controlled validation of road segmentation, metric area and
  white-balance repeatability. Better component quality did not automatically
  produce better held-out-date \pmtwo\ prediction.}
  \label{fig:roadvalidation}
\end{figure}
...
\chapter{Scientific Reframing After Supervisor Review}

\section{Why generic residual correction was conceptually weak}
The earlier architecture predicted total roadside \pmtwo\ with a CNN--RNN and
then trained a tree model on
\begin{equation}
  r_{\mathrm{old}} =
  PM_{\mathrm{observed}}-\widehat{PM}_{\mathrm{image\ base}}.
\end{equation}
This is mathematically valid, but the residual has no single physical meaning.
It can contain regional background, image-model error, traffic, resuspension,
meteorology, sensor noise, date shift and synchronization error. The model
therefore learned to ``correct whatever the image model missed'' rather than
answering which local roadside conditions explained the departure.

\section{Supervisor's physical question and identifiability}
\supervisor\ asked whether traffic, road and image variables were closing a
scientifically meaningful particulate balance or merely acting as generic
predictors. A useful conceptual relation is
\begin{equation}
 PM_{\mathrm{atmospheric}}
 \approx PM_{\mathrm{background}}+PM_{\mathrm{road}}
 +PM_{\mathrm{vehicle}}+PM_{\mathrm{other}}+\epsilon .
\end{equation}
The project did not observe reference values for the road and vehicle terms.
The same total increment can be explained by many combinations; for example,
$30=20+10=5+25$. Exact source contributions are therefore not identifiable
without chemical or source-specific measurements.

\section{Physically structured target}
The revised target is
\begin{equation}
  \Delta PM_{\mathrm{local},t}
  = PM_{\mathrm{sensor},t}-PM_{\mathrm{background},t}.
\end{equation}
For the strongest current experiment, MERRA-2 supplies the external background.
Traffic, road, meteorology, images and geospatial context jointly predict one
local roadside increment. Negative increments are retained because they may
represent cleaner local conditions, background mismatch, spatial-resolution
differences or measurement uncertainty. The safest interpretation is
\emph{local roadside increment relative to MERRA-2 background}, not traffic
pollution alone.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{scientific_reframing.png}
  \caption{Conceptual transition from an unrestricted image-model error to a
  background-anchored local target.}
  \label{fig:reframing}
\end{figure}

\section{Relation to the proposed image--satellite architecture}
The later handwritten proposal suggested learning atmospheric \pmtwo\ from
images and satellite/reanalysis inputs, followed by interpretation using road,
vehicle, OSM and uncertainty information. This was tested as Architecture 2:
target-frame image embeddings and 22 atmospheric variables were fused in a
single ExtraTrees model. Because the training target was still the roadside
sensor, that experiment estimated roadside \pmtwo, not pure atmospheric
\pmtwo. Its results are reported later. The final recommended formulation uses
MERRA-2 as an external anchor and reserves detailed traffic and road variables
for the local-increment branch.
...
\section{TRAQID single-image and engineered-feature baselines}
Two standalone routes were also compared before atmospheric decomposition. A
single front/rear ResNet50 image representation with an MLP achieved
approximately \Rtwo\ 0.49, whereas a Random Forest using roughly 112
engineered traffic, road, weather and time variables achieved approximately
\Rtwo\ 0.84 under the historical split. This established an important design
lesson: for small roadside datasets, explicit traffic and road summaries can
be more useful than generic ImageNet embeddings when used alone. It did not,
however, resolve the later unseen-date validation problem.
...
\section{MUMMA 281-row pilot modelling}
The compact pilot modelling table contained 281 rows, about 190 original
columns and 252 valid image alignments. Twenty-nine ResNet embeddings failed,
and the observations represented only about 48 minutes of a volatile
collection. Reportable predictors excluded all PM and OPC target proxies.

The best fair pilot used a meteorology/gas/time ExtraTrees base on a
log-transformed target and an ExtraTrees residual model using selected road,
traffic and OSM features. It obtained MAE 5.646 \ugm, RMSE 9.719 \ugm\ and
\Rtwo\ 0.650. Small-data temporal approaches were weaker:

\begin{table}[H]
  \centering
  \caption{Compact MUMMA pilot comparison.}
  \begin{tabular}{lrr}
    \toprule
    Model & RMSE & \Rtwo\\
    \midrule
    Optimised non-temporal residual model & 9.72 & 0.650\\
    Hand-engineered T=7 model & 12.09 & 0.466\\
    Best ResNet temporal model & 13.21 & 0.425\\
    Best ResNet50-LSTM & 14.27 & 0.329\\
    \bottomrule
  \end{tabular}
\end{table}

The previous-row persistence baseline reached only about \Rtwo\ 0.466 and most
magnitude predictability disappeared by lag two. This pilot demonstrated that
the dataset was too short and volatile for reliable deep temporal learning.
...
The complete source code, experiment configurations, tests, documentation and
report sources are available in the public GitHub repository
\href{\projectrepository}{Roadside PM Visual Intelligence}.
Large raw datasets, extracted frames, cached geospatial responses, learned
embeddings and trained model binaries are intentionally excluded from Git
because of size, licensing and data-governance constraints. The local artifact
paths below document exact provenance and can be regenerated using the
