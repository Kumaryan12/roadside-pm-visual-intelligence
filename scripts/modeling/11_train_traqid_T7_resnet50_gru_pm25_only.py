...
def make_predictions_df(df, y_true, preds, split_mode, eval_split):
    predictions = df[
        [
            "sequence_id",
            "target_row_id",
    ...
    predictions["split_mode"] = split_mode
    predictions["eval_split"] = eval_split
    predictions[f"actual_{TARGET}"] = y_true.reshape(-1)
    predictions[f"predicted_{TARGET}"] = preds.reshape(-1)
    predictions[f"residual_{TARGET}"] = preds.reshape(-1) - y_true.reshape(-1)
...
def main():
    ...
    parser.add_argument(
        "--out-dir",
        default="experiments/traqid_pretraining_v1/reports/pm25_only_resnet50_concat_gru_image_base_seed42",
    )
    ...
    test_preds_scaled, _ = predict_loader(model, test_loader, device)
    val_preds_scaled, _ = predict_loader(model, val_loader, device)

    test_preds = y_scaler.inverse_transform(test_preds_scaled)
    val_preds = y_scaler.inverse_transform(val_preds_scaled)

    y_test_true = test_df[TARGETS].values
    y_val_true = val_inner[TARGETS].values

    metrics = compute_metrics(y_test_true, test_preds)

    test_predictions = make_predictions_df(
        test_df,
        y_test_true,
        test_preds,
        args.split_mode,
    ...
    val_predictions = make_predictions_df(
        val_inner,
        y_val_true,
        val_preds,
        args.split_mode,
    ...
    metrics["model"] = "resnet50_front_rear_concat_gru_pm25_only"
    ...
    val_predictions_path = out_dir / "val_predictions.csv"
    ...
    print("\n" + "=" * 90)
    print("TRAQID T7 RESNET50 GRU PM2.5-ONLY COMPLETE")
    print("=" * 90)
