"""
Run fine-tuned IDD YOLO11m vehicle detector on processed PM-density frames.

Purpose:
- Use Indian-road-specific detector trained on IDD 15-class dataset.
- Extract auto-rickshaw-aware vehicle counts.
- Save frame-level vehicle features for later PM-density modeling.

Input:
- Old project processed frame manifest:
  /Users/aryansatyendrakumar/Projects/pm_density_image_pipeline/outputs/features/processed_frame_manifest.csv
...
DEFAULT_INPUT_MANIFEST = (
    OLD_PROJECT_ROOT / "outputs/features/processed_frame_manifest.csv"
)

DEFAULT_MODEL_PATH = Path("models/detectors/yolo11m_idd15_v1_best.pt")

DEFAULT_OUTPUT_CSV = Path("outputs/features/idd_vehicle_detections_processed_frames.csv")
...
def extract_features_from_prediction(pred, row, save_annotated: bool, annotated_dir: Path) -> dict:
    result = initialize_feature_row(row)
    ...
    result["idd_vehicle_box_area_ratio"] = float(total_box_area / image_area) if image_area > 0 else 0.0
    result["idd_average_confidence"] = float(sum(confidences) / len(confidences)) if confidences else 0.0
    result["idd_max_confidence"] = float(max(confidences)) if confidences else 0.0
...
def process_frame(model: YOLO, row, conf: float, imgsz: int, save_annotated: bool, annotated_dir: Path) -> dict:
...
def main():
    ...
    parser.add_argument("--annotated-limit", type=int, default=50)
    ...
    annotated_saved = sum(
        ...
        if str(row.get("idd_annotated_image_path", "")).strip()
    )
    ...
    print("\nDetection status:")
    print(out_df["idd_detection_status"].value_counts())
